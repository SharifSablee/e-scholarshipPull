...
//livechat 
    ...
    socket.on('roomUsers', ({ room, users, role }) => {
        outputRoomName(room);
        outputUsers(users, role);
    });
    ...
    // Add users to DOM
    function outputUsers(users, role) {
        userList.innerHTML = '';
        users.forEach((user) => {
            if (user.role == "insitution") {
                const li = document.createElement('li');
                li.innerText = (user.username+" (Room Master)");
    ...
    document.getElementById('leave-btn').addEventListener('click', () => {
        ...
        if (leaveRoom) {
            window.location = '../student_page/student_JoinChatRoom.html';
        } 
    });
